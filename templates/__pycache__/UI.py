...
def monte_carlo_simulation(stock_symbol, mc_sims, T):
...
def stock():
    ...
    mc_sims = int(request.form['simulations'])  # Get number of simulations from form
    T = int(request.form['timeframe'])  # Get timeframe in days from form
    mean_portfolio_value, std_portfolio_value = monte_carlo_simulation(ticker.upper(), mc_sims, T)
